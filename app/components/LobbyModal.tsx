...
const LobbyModal = ({ isOpen, onClose }: LobbyModalProps) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900/95 backdrop-blur-md rounded-lg shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto">
                {/* Modal Header */}
                    ...
                    <div>
                        ...
                        {currentLobby && (
                            <p className="text-sm text-blue-400 mt-1">
                                Currently in: <span className="font-semibold">{currentLobby.name}</span>
                            </p>
                        )}
                    </div>
                    <Button
                ...
                <div className="p-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-8">
                        {availableLobbies.map((lobby) => (
                            <Card key={lobby.lobbyId} className="bg-gray-800/90 border-gray-600 hover:border-blue-500 transition-all duration-300">
                                <CardHeader>
                                    <div className="flex items-start justify-between">
                                        <div>
                                            <CardTitle className={`text-xl ${
                                                lobby.lobbyId === 'hack-nation' ? 'text-green-400' : 'text-blue-400'
                                            } ${currentLobby?.lobbyId === lobby.lobbyId ? 'opacity-60' : ''}`}>
                                                {lobby.name} {currentLobby?.lobbyId === lobby.lobbyId && '(Current)'}
                                            </CardTitle>
                                ...
                                <CardContent>
                                    <div className="space-y-4">
                                        ...
                                        <Button 
                                            ...
                                            } ${currentLobby?.lobbyId === lobby.lobbyId ? 'opacity-50' : ''}`}
                                            onClick={() => handleJoinLobby(lobby.lobbyId)}
                                            disabled={lobby.currentPlayers.length >= lobby.maxPlayers || currentLobby?.lobbyId === lobby.lobbyId}
                                        >
                                            {currentLobby?.lobbyId === lobby.lobbyId ? 'Current World' :
                    ...
                    <div className="text-center">
                        ...
                        {showDirectJoin && (
                            <div className="bg-gray-800/90 rounded-lg p-4 max-w-md mx-auto">
